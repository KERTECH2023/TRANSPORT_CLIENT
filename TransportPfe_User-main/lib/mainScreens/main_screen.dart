    ...
    referenceRideRequest!.onValue.listen((eventSnap) async {
      ...
      if ((snapshot.value as Map)["carDetails"] != null) {
        ...
        setState(() {
          ...
          carNumber = (snapshot.value as Map)["carDetails"]
                  ["carImmatriculation"]
              .toString();
          carType =
              (snapshot.value as Map)["carDetails"]["carModel"].toString();
        ...
        // }
          ...
          double fareAmount =
              await AssistantMethods.calculateFareAmountFromSourceToDestination(
                  tripDirectionDetailsInfo!, carType);

          // Enregistrer le montant du trajet dans la base de données
          referenceRideRequest!.update({
            "fareAmount": fareAmount,
          }).then((_) {
            print("Montant du trajet enregistré avec succès: $fareAmount");
          }).catchError((error) {
            print(
                "Erreur lors de l'enregistrement du montant du trajet: $error");
          });
        }
            ...
            var response = await showDialog(
                ...
                builder: (BuildContext context) {
                  return PayFareDialog(
                    fareAmount:
                        double.parse(fareAmount.toString().substring(0, 3)),
